...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/DogetoreumParams"
else
    PARAMS_DIR="$HOME/.dogetoreum-params"
fi
...
DOWNLOAD_URL="https://github.com/DogeToreum/dogetoreum/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with DOGUM_DISABLE_SOMETHING=1
DOGUM_DISABLE_WGET="${DOGUM_DISABLE_WGET:-}"
#DOGUM_DISABLE_IPFS="${DOGUM_DISABLE_IPFS:-}"
DOGUM_DISABLE_CURL="${DOGUM_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$DOGUM_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$DOGUM_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$DOGUM_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Dogetoreum parameters!
...
main() {
    ...
    cat <<EOF
DOGUM - fetch-params.sh

This script will fetch the Dogetoreum SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Dogetoreum SNARK parameters. Note that it is
