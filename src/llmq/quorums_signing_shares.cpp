...
namespace llmq {
    ...
    std::string CSigSesAnn::ToString() const {
        return strprintf("sessionId=%d, llmqType=%d, quorumHash=%s, id=%s, msgHash=%s",
                         sessionId, getLlmqType(), getQuorumHash().ToString(), getId().ToString(),
                         getMsgHash().ToString());
    }
...
//////////////////////
    ...
    void CSigSharesManager::StartWorkerThread() {
        ...
        workThread = std::thread(&TraceThread < std::function < void() > > , "sigshares",
    ...
    void CSigSharesManager::ProcessMessage(const CNode *pfrom, const std::string &strCommand, CDataStream &vRecv) {
        ...
        if (sporkManager.IsSporkActive(SPORK_23_QUORUM_ALL_CONNECTED) && strCommand == NetMsgType::QSIGSHARE) {
    ...
    bool CSigSharesManager::ProcessMessageSigSesAnn(const CNode *pfrom, const CSigSesAnn &ann) {
        ...
        if (ann.getSessionId() == UNINITIALIZED_SESSION_ID || ann.getQuorumHash().IsNull() || ann.getId().IsNull() ||
            ann.getMsgHash().IsNull()) {
            return false;
        }

        LogPrint(BCLog::LLMQ_SIGS, "CSigSharesManager::%s -- ann={%s}, node=%d\n", __func__, ann.ToString(),
                 pfrom->GetId());
    ...
    bool
    CSigSharesManager::ProcessMessageBatchedSigShares(const CNode *pfrom, const CBatchedSigShares &batchedSigShares) {
        CSigSharesNodeState::SessionInfo sessionInfo;
        ...
        LogPrint(BCLog::LLMQ_SIGS, "CSigSharesManager::%s -- signHash=%s, shares=%d, new=%d, inv={%s}, node=%d\n",
                 __func__,
                 sessionInfo.signHash.ToString(), batchedSigShares.sigShares.size(), sigSharesToProcess.size(),
                 batchedSigShares.ToInvString(), pfrom->GetId());
    ...
    void CSigSharesManager::ProcessMessageSigShare(NodeId fromId, const CSigShare &sigShare) {
        ...
        LogPrint(BCLog::LLMQ_SIGS, "CSigSharesManager::%s -- signHash=%s, id=%s, msgHash=%s, member=%d, node=%d\n",
                 __func__,
                 sigShare.GetSignHash().ToString(), sigShare.getId().ToString(), sigShare.getMsgHash().ToString(),
    ...
    bool CSigSharesManager::PreVerifyBatchedSigShares(const CSigSharesNodeState::SessionInfo &session,
        ...
        if (!session.quorum->HasVerificationVector()) {
            // TODO we should allow to ask other nodes for the quorum vvec if we missed it in the DKG
            LogPrint(BCLog::LLMQ_SIGS,
                     "CSigSharesManager::%s -- we don't have the quorum vvec for %s, no verification possible.\n",
                     __func__,
    ...
    bool CSigSharesManager::ProcessPendingSigShares(const CConnman &connman) {
        ...
        for (const auto &[nodeId, v]: sigSharesByNodes) {
            for (const auto &sigShare: v) {
                ...
                batchVerifier.PushMessage(nodeId, sigShare.GetKey(), sigShare.GetSignHash(), sigShare.sigShare.Get(),
                                          pubKeyShare);
                verifyCount++;
            }
        }
...
// sig shares are already verified when entering this method
    void
    CSigSharesManager::ProcessSigShare(const CSigShare &sigShare, const CConnman &connman, const CQuorumCPtr &quorum) {
    ...
    void CSigSharesManager::TryRecoverSig(const CQuorumCPtr &quorum, const uint256 &id, const uint256 &msgHash) {
        ...
        std::vector <CBLSId> idsForRecovery;
        {
            ...
            for (auto it = sigSharesForSignHash->begin(); it != sigSharesForSignHash->end() &&
                                                          sigSharesForRecovery.size() <
                                                          size_t(quorum->params.threshold); ++it) {
    ...
    CSigSharesManager::SelectMemberForRecovery(const CQuorumCPtr &quorum, const uint256 &id, size_t attempt) {
    ...
    void CSigSharesManager::CollectSigSharesToSend(
    ...
    void CSigSharesManager::CollectSigSharesToSendConcentrated(
            std::unordered_map <NodeId, std::vector<CSigShare>> &sigSharesToSend, const std::vector<CNode *> &vNodes) {
        AssertLockHeld(cs);
        ...
        for (auto &[_, signedSession]: signedSessions) {
            ...
            if (curTime >= signedSession.nextAttemptTime) {
                ...
                auto dmn = SelectMemberForRecovery(signedSession.quorum, signedSession.sigShare.getId(),
                                                   signedSession.attempt);
                signedSession.attempt++;

                LogPrint(BCLog::LLMQ_SIGS, "CSigSharesManager::%s -- signHash=%s, sending to %s, attempt=%d\n",
                         __func__,
                         signedSession.sigShare.GetSignHash().ToString(), dmn->proTxHash.ToString(),
                         signedSession.attempt);
    ...
    void CSigSharesManager::CollectSigSharesToAnnounce(
        ...
        sigSharesQueuedToAnnounce.ForEach(
                [this, &quorumNodesMap, &sigSharesToAnnounce](const SigShareKey &sigShareKey, bool) {
                    ...
                    if (it == quorumNodesMap.end()) {
                        ...
                        it = quorumNodesMap.emplace(std::piecewise_construct, std::forward_as_tuple(quorumKey),
                                                    std::forward_as_tuple(nodeIds.begin(), nodeIds.end())).first;
                    }
    ...
    bool CSigSharesManager::GetSessionInfoByRecvId(NodeId nodeId, uint32_t sessionId,
                                                   CSigSharesNodeState::SessionInfo &retInfo) {
        LOCK(cs);
        return nodeStates[nodeId].GetSessionInfoByRecvId(sessionId, retInfo);
    }

    CSigShare CSigSharesManager::RebuildSigShare(const CSigSharesNodeState::SessionInfo &session,
                                                 const CBatchedSigShares &batchedSigShares, size_t idx) {
            ...
            for (auto &signHash: timeoutSessions) {
                if (const size_t count = sigShares.CountForSignHash(signHash); count > 0) {
                    ...
                    if (LogAcceptCategory(BCLog::LLMQ_SIGS)) {
                        if (const auto quorumIt = quorums.find(
                                    std::make_pair(oneSigShare.getLlmqType(), oneSigShare.getQuorumHash())); quorumIt !=
                                                                                                             quorums.end()) {
                    ...
                    LogPrint(BCLog::LLMQ_SIGS,
                             ...
                             __func__,
                             signHash.ToString(), oneSigShare.getId().ToString(), oneSigShare.getMsgHash().ToString(),
                             count, strMissingMembers);
                } else {
                    LogPrint(BCLog::LLMQ_SIGS,
    ...
    CSigSharesManager::CreateSigShare(const CQuorumCPtr &quorum, const uint256 &id, const uint256 &msgHash) const {
        cxxtimer::Timer t(true);
        auto activeSmartNodeProTxHash = WITH_LOCK(activeSmartnodeInfoCs,
        return activeSmartnodeInfo.proTxHash);
        ...
        LogPrint(BCLog::LLMQ_SIGS,
                 ...
                 quorum->params.type, quorum->qc->quorumHash.ToString(), t.count());
...
// causes all known sigShares to be re-announced
    void
    CSigSharesManager::ForceReAnnouncement(const CQuorumCPtr &quorum, Consensus::LLMQType llmqType, const uint256 &id,
