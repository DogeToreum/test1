...
namespace llmq {
    ...
    int32_t CChainLockSig::getHeight() const { return nHeight; }

    const uint256 &CChainLockSig::getBlockHash() const { return blockHash; }

    const CBLSSignature &CChainLockSig::getSig() const { return sig; }

    bool CChainLockSig::IsNull() const { return nHeight == -1 && blockHash == uint256(); }
    ...
    CChainLocksHandler::CChainLocksHandler(CTxMemPool &_mempool, CConnman &_connman)
            : scheduler(std::make_unique<CScheduler>()),
              ...
              scheduler_thread(std::make_unique<std::thread>(
                      [&] { TraceThread("cl-schdlr", [&] { scheduler->serviceQueue(); }); })) {
    }
    ...
    void
    CChainLocksHandler::ProcessNewChainLock(const NodeId from, const llmq::CChainLockSig &clsig, const uint256 &hash) {
        CheckActiveState();
        ...
        if (!llmq::CSigningManager::VerifyRecoveredSig(Params().GetConsensus().llmqTypeChainLocks, clsig.getHeight(),
                                                       requestId, clsig.getBlockHash(), clsig.getSig())) {
        ...
        CBlockIndex * pindex = WITH_LOCK(cs_main,
        return LookupBlockIndex(clsig.getBlockHash()));
                ...
                for (auto &txid: *txids) {
                    ...
                    if (txAge < WAIT_FOR_ISLOCK_TIMEOUT && !quorumInstantSendManager->IsLocked(txid)) {
                        LogPrint(BCLog::CHAINLOCKS,
                                 "CChainLocksHandler::%s -- not signing block %s due to TX %s not being islocked and not old enough. age=%d\n",
                                 __func__,
    ...
    void CChainLocksHandler::BlockConnected(const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindex,
        ...
        if (it == blockTxs.end()) {
            ...
            it = blockTxs.emplace(pindex->GetBlockHash(), std::make_shared < std::unordered_set < uint256,
                                  StaticSaltedHasher >> ()).first;
        }
    ...
    CChainLocksHandler::BlockTxs::mapped_type CChainLocksHandler::GetBlockTxs(const uint256 &blockHash) {
        ...
        if (!ret) {
            ...
            LogPrint(BCLog::CHAINLOCKS,
                     "CChainLocksHandler::%s -- blockTxs for %s not found. Trying ReadBlockFromDisk\n", __func__,
                     blockHash.ToString());
...
// This should also not be called from validation signals, as this might result in recursive calls
    void CChainLocksHandler::EnforceBestChainLock() {
        ...
        const auto &params = Params();
        {
            ...
            activateNeeded = ::ChainActive().Tip()->GetAncestor(currentBestChainLockBlockIndex->nHeight) !=
        ...
        const CBlockIndex *pindexNotify = nullptr;
        {
            ...
            if (lastNotifyChainLockBlockIndex != currentBestChainLockBlockIndex &&
                ::ChainActive().Tip()->GetAncestor(currentBestChainLockBlockIndex->nHeight) ==
                currentBestChainLockBlockIndex) {
    ...
    void CChainLocksHandler::Cleanup() {
        ...
        for (auto it = txFirstSeenTime.begin(); it != txFirstSeenTime.end();) {
            uint256 hashBlock;
            CTransactionRef tx = GetTransaction(/* block_index */ nullptr, &mempool, it->first, Params().GetConsensus(),
                                                                  hashBlock);
            if (!tx) {
                ...
                if (::ChainActive().Tip()->GetAncestor(pindex->nHeight) == pindex &&
                    ::ChainActive().Height() - pindex->nHeight >= 6) {
