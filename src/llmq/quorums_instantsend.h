...
namespace llmq {
    ...
    private:
        ...
        int best_confirmed_height
        GUARDED_BY(cs_db) {0};

        std::unique_ptr <CDBWrapper> db
        GUARDED_BY(cs_db) {nullptr};
        ...
        EXCLUSIVE_LOCKS_REQUIRED(cs_db);
         ...
         */
        void
        RemoveInstantSendLock(CDBBatch &batch, const uint256 &hash, CInstantSendLockPtr islock, bool keep_cache = true)
    ...
    public:
        explicit CInstantSendDb(bool unitTests, bool fWipe) :
                db(std::make_unique<CDBWrapper>(unitTests ? "" : (GetDataDir() / "llmq/isdb"), 32 << 20, unitTests,
                                                fWipe)) {}

        void Upgrade()

        LOCKS_EXCLUDED(cs_db);
        ...
        void
        RemoveBlockInstantSendLocks(const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindexDisconnected)
    ...
    private:
        ...
        GUARDED_BY(cs_pendingLocks);
        // Tried to verify but there is no tx yet.
        std::unordered_map <uint256, std::pair<NodeId, CInstantSendLockPtr>, StaticSaltedHasher> pendingNoTxInstantSendLocks
        GUARDED_BY(cs_pendingLocks);
        ...
        GUARDED_BY(cs_pendingRetry);

    public:
        explicit CInstantSendManager(CTxMemPool &_mempool, CConnman &_connman, bool unitTests, bool fWipe) : db(
                unitTests, fWipe), mempool(_mempool), connman(_connman) { workInterrupt.reset(); }
    ...
    private:
        ...
        LOCKS_EXCLUDED(cs_creating, cs_pendingLocks
        );
        ...
        std::unordered_set <uint256> ProcessPendingInstantSendLocks(int signOffset,
                                                                    const std::unordered_map <uint256, std::pair<NodeId, CInstantSendLockPtr>, StaticSaltedHasher> &pend,
                                                                    bool ban)

        LOCKS_EXCLUDED(cs_pendingLocks);
        ...
        LOCKS_EXCLUDED(cs_pendingLocks, cs_nonLocked
        );

        void RemoveNonLockedTx(const uint256 &txid, bool retryChildren)

        LOCKS_EXCLUDED(cs_nonLocked, cs_pendingRetry
        );
        ...
        LOCKS_EXCLUDED(cs_creating, cs_nonLocked, cs_pendingRetry
        );
    ...
    public:
        ...
        LOCKS_EXCLUDED(cs_inputRequests, cs_creating
        );
