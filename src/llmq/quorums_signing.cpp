...
namespace llmq {
    ...
    void CRecoveredSigsDb::MigrateRecoveredSigs() {
        ...
        LogPrintf("CRecoveredSigsDb::%d -- done\n", __func__);
    }

    bool
    CRecoveredSigsDb::HasRecoveredSig(Consensus::LLMQType llmqType, const uint256 &id, const uint256 &msgHash) const {
    ...
    bool
    CRecoveredSigsDb::GetRecoveredSigById(Consensus::LLMQType llmqType, const uint256 &id, CRecoveredSig &ret) const {
    ...
    void CRecoveredSigsDb::WriteRecoveredSig(const llmq::CRecoveredSig &recSig) {
        ...
        auto k5 = std::make_tuple(std::string("rs_t"), (uint32_t) htobe32(curTime), recSig.getLlmqType(),
                                  recSig.getId());
    ...
    void CRecoveredSigsDb::RemoveRecoveredSig(CDBBatch &batch, Consensus::LLMQType llmqType, const uint256 &id,
                                              bool deleteHashKey, bool deleteTimeKey) {
        AssertLockHeld(cs);
        ...
        if (deleteTimeKey) {
            ...
            if (db->ReadDataStream(k2, writeTimeDs) && writeTimeDs.size() == sizeof(uint32_t)) {
                ...
                auto k5 = std::make_tuple(std::string("rs_t"), (uint32_t) htobe32(writeTime), recSig.getLlmqType(),
                                          recSig.getId());
                batch.Erase(k5);
            }
        }
...
//////////////////
    ...
    void CSigningManager::ProcessMessageRecoveredSig(CNode *pfrom,
        ...
        LogPrint(BCLog::LLMQ, "CSigningManager::%s -- signHash=%s, id=%s, msgHash=%s, node=%d\n", __func__,
                 recoveredSig->buildSignHash().ToString(), recoveredSig->getId().ToString(),
                 recoveredSig->getMsgHash().ToString(), pfrom->GetId());

        LOCK(cs);
        if (pendingReconstructedRecoveredSigs.count(recoveredSig->GetHash())) {
            // no need to perform full verification
            LogPrint(BCLog::LLMQ,
                     "CSigningManager::%s -- already pending reconstructed sig, signHash=%s, id=%s, msgHash=%s, node=%d\n",
                     __func__,
                     recoveredSig->buildSignHash().ToString(), recoveredSig->getId().ToString(),
                     recoveredSig->getMsgHash().ToString(), pfrom->GetId());
            return;
    ...
    void CSigningManager::ProcessPendingReconstructedRecoveredSigs() {
        decltype(pendingReconstructedRecoveredSigs)
        m;
        {
            LOCK(cs);
    ...
    bool CSigningManager::ProcessPendingRecoveredSigs() {
        ...
        for (const auto &p: recSigsByNode) {
            ...
            for (const auto &recSig: v) {
                ...
                batchVerifier.PushMessage(nodeId, recSig->GetHash(), recSig->buildSignHash(), recSig->sig.Get(),
...
// signature must be verified already
                ...
                if (db.GetRecoveredSigById(llmqType, recoveredSig->getId(), otherRecoveredSig)) {
                    ...
                    if (signHash != otherSignHash) {
                        ...
                        LogPrintf(
                                ...
                                __func__,
                                signHash.ToString(), recoveredSig->getId().ToString(),
                                recoveredSig->getMsgHash().ToString(), otherSignHash.ToString());
                    } else {
    ...
    void
    CSigningManager::PushReconstructedRecoveredSig(const std::shared_ptr<const llmq::CRecoveredSig> &recoveredSig) {
        LOCK(cs);
        pendingReconstructedRecoveredSigs.emplace(std::piecewise_construct,
                                                  std::forward_as_tuple(recoveredSig->GetHash()),
                                                  std::forward_as_tuple(recoveredSig));
    }
    ...
    bool CSigningManager::AsyncSignIfMember(Consensus::LLMQType llmqType, const uint256 &id, const uint256 &msgHash,
                                            const uint256 &quorumHash, bool allowReSign) {
    ...
    bool
    CSigningManager::HasRecoveredSig(Consensus::LLMQType llmqType, const uint256 &id, const uint256 &msgHash) const {
    ...
    bool CSigningManager::GetRecoveredSigForId(Consensus::LLMQType llmqType, const uint256 &id,
                                               llmq::CRecoveredSig &retRecSig) const {
    ...
    CQuorumCPtr
    CSigningManager::SelectQuorumForSigning(Consensus::LLMQType llmqType, const uint256 &selectionHash, int signHeight,
                                            int signOffset) {
    ...
    bool CSigningManager::VerifyRecoveredSig(Consensus::LLMQType llmqType, int signedAtHeight, const uint256 &id,
                                             const uint256 &msgHash, const CBLSSignature &sig, const int signOffset) {
