...
// - commit-lie
    ...
    bool CDKGSession::Init(const CBlockIndex *_pQuorumBaseBlockIndex, const std::vector <CDeterministicMNCPtr> &mns,
        ...
        if (!myProTxHash.IsNull()) {
            quorumDKGDebugManager->InitLocalSessionStatus(params, m_quorum_base_block_index->GetBlockHash(),
    ...
    void CDKGSession::SendContributions(CDKGPendingMessages &pendingMessages) {
        ...
        for (size_t i = 0; i < members.size(); i++) {
            ...
            int minSmartnodeProtoVersion = Params().IsFutureActive(::ChainActive().Tip()) ? MIN_SMARTNODE_PROTO_VERSION
                                                                                          : OLD_MIN_SMARTNODE_PROTO_VERSION;
            if (!qc.contributions->Encrypt(i, m->dmn->pdmnState->pubKeyOperator.Get(), skContrib,
                                           minSmartnodeProtoVersion)) {
        ...
        return activeSmartnodeInfo.blsKeyOperator->Sign(qc.GetSignHash()));
...
// only performs cheap verifications, but not the signature of the message. this is checked with batched verification
    bool CDKGSession::PreVerifyMessage(const CDKGContribution &qc, bool &retBan) const {
        ...
        if (member->contributions.size() >= 2) {
            ...
            logger.Batch("dropping contribution from %s as we already got %d contributions",
    ...
    void CDKGSession::ReceiveMessage(const CDKGContribution &qc, bool &retBan) {
        ...
        logger.Batch("received and relayed contribution. received=%d/%d, time=%d", receivedCount, members.size(),
                     t1.count());
        ...
        int minSmartnodeProtoVersion = Params().IsFutureActive(::ChainActive().Tip()) ? MIN_SMARTNODE_PROTO_VERSION
                                                                                      : OLD_MIN_SMARTNODE_PROTO_VERSION;
        if (!qc.contributions->Decrypt(*myIdx, WITH_LOCK(activeSmartnodeInfoCs,
            return *activeSmartnodeInfo.blsKeyOperator), skContribution, PROTOCOL_VERSION)) {
        ...
        if (complain) {
            ...
            quorumDKGDebugManager->UpdateLocalMemberStatus(params.type, member->idx,
                                                           [&](CDKGDebugMemberStatus &status) {
    ...
    void CDKGSession::VerifyPendingContributions() {
        ...
        for (const auto &idx: pend) {
            ...
            dkgManager.WriteEncryptedContributions(params.type, m_quorum_base_block_index, m->dmn->proTxHash,
                                                   *vecEncryptedContributions[idx]);
        }

        auto result = blsWorker.VerifyContributionShares(myId, vvecs, skContributions);
        if (result.size() != memberIndexes.size()) {
            logger.Batch(
                    "VerifyContributionShares returned result of size %d but size %d was expected, something is wrong",
    ...
    void CDKGSession::VerifyConnectionAndMinProtoVersions() const {
        ...
        for (const auto &m: members) {
            ...
            if (auto it = protoMap.find(m->dmn->proTxHash); it == protoMap.end()) {
                ...
                logger.Batch("%s is not connected to us, badConnection=%b", m->dmn->proTxHash.ToString(),
                             m->badConnection);
            } else if (it->second < MIN_SMARTNODE_PROTO_VERSION) {
                m->badConnection = true;
                logger.Batch("%s does not have min proto version %d (has %d)", m->dmn->proTxHash.ToString(),
                             MIN_SMARTNODE_PROTO_VERSION, it->second);
            }

            auto lastOutbound = mmetaman.GetMetaInfo(m->dmn->proTxHash)->GetLastOutboundSuccess();
            if (GetAdjustedTime() - lastOutbound > 60 * 60) {
                m->badConnection = true;
                logger.Batch("%s no outbound connection since %d seconds", m->dmn->proTxHash.ToString(),
...
// only performs cheap verifications, but not the signature of the message. this is checked with batched verification
    ...
    void CDKGSession::ReceiveMessage(const CDKGComplaint &qc, bool &retBan) {
        ...
        for (size_t i = 0; i < members.size(); i++) {
            const auto &m = members[i];
            if (qc.badMembers[i]) {
                logger.Batch("%s voted for %s to be bad", member->dmn->proTxHash.ToString(),
    ...
    void CDKGSession::VerifyAndJustify(CDKGPendingMessages &pendingMessages) {
        ...
        for (const auto &m: members) {
            ...
            if (m->badMemberVotes.size() >= size_t(params.dkgBadVotesThreshold)) {
                logger.Batch("%s marked as bad as %d other members voted for this", m->dmn->proTxHash.ToString(),
                             m->badMemberVotes.size());
                MarkBadMember(m->idx);
                continue;
            }
            if (m->complaints.empty()) {
            ...
            const auto &qc = WITH_LOCK(invCs,
            return std::move(complaints.at(*m->complaints.begin())));
    ...
    void CDKGSession::SendJustification(CDKGPendingMessages &pendingMessages, const std::set <uint256> &forMembers) {
        ...
        qj.sig = WITH_LOCK(activeSmartnodeInfoCs,
        return activeSmartnodeInfo.blsKeyOperator->Sign(qj.GetSignHash()));
    ...
    void CDKGSession::ReceiveMessage(const CDKGJustification &qj, bool &retBan) {
        ...
        for (const auto &p: qj.contributions) {
            ...
            futures.emplace_back(
                    blsWorker.AsyncVerifyContributionShare(member2->id, receivedVvecs[member->idx], skContribution));
        }
        ...
        for (const auto &p: qj.contributions) {
            ...
            if (!result) {
                logger.Batch("  %s did send an invalid justification for %s", member->dmn->proTxHash.ToString(),
                             member2->dmn->proTxHash.ToString());
                MarkBadMember(member->idx);
            } else {
                logger.Batch("  %s justified for %s", member->dmn->proTxHash.ToString(),
                             member2->dmn->proTxHash.ToString());
                if (AreWeMember() && member2->id == myId) {
                    ...
                    dkgManager.WriteVerifiedSkContribution(params.type, m_quorum_base_block_index,
                                                           member->dmn->proTxHash, skContribution);
                }
...
// only performs cheap verifications, but not the signature of the message. this is checked with batched verification
    ...
    void CDKGSession::ReceiveMessage(const CDKGPrematureCommitment &qc, bool &retBan) {
        ...
        logger.Batch("received premature commitment from %s. validMembers=%d", qc.proTxHash.ToString(),
        ...
        if (quorumVvec == nullptr) {
            ...
            CBLSPublicKey pubKeyShare = cache.BuildPubKeyShare(
                    ::SerializeHash(std::make_pair(memberIndexes, member->id)), quorumVvec, member->id);
        ...
        logger.Batch("verified premature commitment. received=%d/%d, time=%d", receivedCount, members.size(),
                     t1.count());
    }
    ...
    std::vector <CFinalCommitment> CDKGSession::FinalizeCommitments() {
        ...
        for (const auto &p: commitmentsMap) {
            ...
            logger.Batch(
                    "final commitment: validMembers=%d, signers=%d, quorumPublicKey=%s, time1=%d, time2=%d, time3=%d",
    ...
    void CDKGSession::RelayInvToParticipants(const CInv &inv) const {
        connman.ForEachNode([&](CNode *pnode) {
            if (pnode->qwatch || (!pnode->GetVerifiedProRegTxHash().IsNull() &&
                                  (relayMembers.count(pnode->GetVerifiedProRegTxHash()) != 0))) {
