...
namespace llmq {
    ...
    void CQuorumBlockProcessor::ProcessMessage(CNode *pfrom, const std::string &strCommand, CDataStream &vRecv) {
        ...
        const CBlockIndex *pQuorumBaseBlockIndex;
        {
            ...
            int quorumHeight =
                    pQuorumBaseBlockIndex->nHeight - (pQuorumBaseBlockIndex->nHeight % GetLLMQParams(type).dkgInterval);
...

        {
            ...
            LOCK(mineableCommitmentsCs);
            auto k = std::make_pair(type, qc.quorumHash);
            auto it = mineableCommitmentsByQuorum.find(k);
            if (it != mineableCommitmentsByQuorum.end()) {
                auto jt = mineableCommitments.find(it->second);
                if (jt != mineableCommitments.end() && jt->second.CountSigners() <= qc.CountSigners()) {
                    return;
                }
            }
        }
        ...
        if (!qc.Verify(pQuorumBaseBlockIndex, true)) {
            LogPrint(BCLog::LLMQ, "CQuorumBlockProcessor::%s -- commitment for quorum %s:%d is not valid, peer=%d\n",
                     __func__, qc.quorumHash.ToString(), qc.llmqType, pfrom->GetId());
            WITH_LOCK(cs_main, Misbehaving(pfrom->GetId(), 100));
            return;
        }

        LogPrint(BCLog::LLMQ,
                 "CQuorumBlockProcessor::%s -- received commitment for quorum %s:%d, validMembers=%d, signers=%d, peer=%d\n",
                 __func__,
...
// minedHeight is inversed so that entries are traversable in reversed order
    static std::tuple <std::string, Consensus::LLMQType, uint32_t>
    BuildInversedHeightKey(Consensus::LLMQType llmqType, int nMinedHeight) {
    ...
    bool CQuorumBlockProcessor::ProcessCommitment(int nHeight, const uint256 &blockHash, const CFinalCommitment &qc,
                                                  CValidationState &state, bool fJustCheck, bool fBLSChecks) {
        AssertLockHeld(cs_main);
...

        {
            ...
            mineableCommitmentsByQuorum.erase(cacheKey);
            mineableCommitments.erase(::SerializeHash(qc));
        }

        LogPrint(BCLog::LLMQ,
                 "CQuorumBlockProcessor::%s -- processed commitment from block. type=%d, quorumHash=%s, signers=%s, validMembers=%d, quorumPublicKey=%s\n",
                 __func__,
                 qc.llmqType, quorumHash.ToString(), qc.CountSigners(), qc.CountValidMembers(),
                 qc.quorumPublicKey.ToString());
...
// TODO remove this with 0.15.0
    bool CQuorumBlockProcessor::UpgradeDB() {
        ...
        uint256 bestBlock;
        if (evoDb.GetRawDB().Read(DB_BEST_BLOCK_UPGRADE, bestBlock) &&
            bestBlock == ::ChainActive().Tip()->GetBlockHash()) {
            return true;
        }
        ...
        if (::ChainActive().Height() >= 1) {
            ...
            while (pindex) {
                ...
                for (const auto &p: qcs) {
                    ...
                    auto pQuorumBaseBlockIndex = LookupBlockIndex(qc.quorumHash);
                    evoDb.GetRawDB().Write(
                            std::make_pair(DB_MINED_COMMITMENT, std::make_pair(qc.llmqType, qc.quorumHash)),
                            std::make_pair(qc, pindex->GetBlockHash()));
                    evoDb.GetRawDB().Write(BuildInversedHeightKey(qc.llmqType, pindex->nHeight),
                                           pQuorumBaseBlockIndex->nHeight);
                }
    ...
    bool CQuorumBlockProcessor::GetCommitmentsFromBlock(const CBlock &block, const CBlockIndex *pindex,
...
// WARNING: This method returns uint256() on the first block of the DKG interval (because the block hash is not known yet)
    ...
    CFinalCommitmentPtr
    CQuorumBlockProcessor::GetMinedCommitment(Consensus::LLMQType llmqType, const uint256 &quorumHash,
                                              uint256 &retMinedBlockHash) const {
...
// The returned quorums are in reversed order, so the most recent one is at index 0
    ...
    CQuorumBlockProcessor::GetMinedCommitmentsUntilBlock(Consensus::LLMQType llmqType, const CBlockIndex *pindex,
...
// The returned quorums are in reversed order, so the most recent one is at index 0
    ...
    CQuorumBlockProcessor::GetMinedAndActiveCommitmentsUntilBlock(const CBlockIndex *pindex) const {
    ...
    bool CQuorumBlockProcessor::HasMineableCommitment(const uint256 &hash) const {
        LOCK(mineableCommitmentsCs);
        return mineableCommitments.count(hash) != 0;
    }
    ...
    void CQuorumBlockProcessor::AddMineableCommitment(const CFinalCommitment &fqc) {
...

        {
            ...
            auto ins = mineableCommitmentsByQuorum.emplace(k, commitmentHash);
            if (ins.second) {
                mineableCommitments.emplace(commitmentHash, fqc);
                relay = true;
            } else {
                const auto &oldFqc = mineableCommitments.at(ins.first->second);
                if (fqc.CountSigners() > oldFqc.CountSigners()) {
                    ...
                    mineableCommitments.erase(ins.first->second);
                    mineableCommitments.emplace(commitmentHash, fqc);
                    relay = true;
                }
            }
    ...
    bool CQuorumBlockProcessor::GetMineableCommitmentByHash(const uint256 &commitmentHash,
                                                            llmq::CFinalCommitment &ret) const {
        LOCK(mineableCommitmentsCs);
        auto it = mineableCommitments.find(commitmentHash);
        if (it == mineableCommitments.end()) {
            return false;
        }
        ret = it->second;
        return true;
...
// Will return true and a null commitment if no mineable commitment is known and none was mined yet
    bool CQuorumBlockProcessor::GetMineableCommitment(const Consensus::LLMQParams &llmqParams, int nHeight,
                                                      CFinalCommitment &ret) const {
        AssertLockHeld(cs_main);
        ...
        ret = mineableCommitments.at(it->second);

        return true;
    }

    bool CQuorumBlockProcessor::GetMineableCommitmentTx(const Consensus::LLMQParams &llmqParams, int nHeight,
