...
namespace {
    ...
    void RandAddSeedPerfmon(CSHA512 &hasher) {
        ...
        // Initialize last_perform to 0 seconds, we don't skip the first call.
 ...
 */
    ...
    CSHA512 &operator<<(CSHA512 &hasher, const T &data) {
        ...
        static_assert(!std::is_same<typename std::decay<T>::type, unsigned char *>::value,
                      "Calling operator<<(CSHA512, unsigned char*) is probably not what you want");
        static_assert(!std::is_same<typename std::decay<T>::type, const char *>::value,
                      "Calling operator<<(CSHA512, const char*) is probably not what you want");
        static_assert(!std::is_same<typename std::decay<T>::type, const unsigned char *>::value,
    ...
    void AddAllCPUID(CSHA512& hasher)
    {
      ...
      // Iterate over all standarf leaves
      AddCPUID(hasher, 0, 0, ax, bx, cx, dx); // Return max leaf in ax
      uint32_t max = ax;
