...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase,
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in dogetoreum.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in dogetoreum.qrc)
...
static const char *qt_argv = "dogum-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info) {
    ...
    if (success) {
        ...
        // dogetoreum: URIs or payment requests:
        if (paymentServer) {
...
void BitcoinApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr(
            "A fatal error occurred. Dogetoreum Core can no longer continue safely and will quit.") + QString("\n\n") +
                                                        message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs() {
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Dogetoreum Core - \""),
                 ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
...
int GuiMain(int argc, char *argv[]) {
    ...
    Q_INIT_RESOURCE(dogetoreum);
    Q_INIT_RESOURCE(dogetoreum_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse dogetoreum.conf
    ...
    // dogetoreum: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
