...
Intro::Intro(QWidget *parent, uint64_t blockchain_size, uint64_t chain_state_size) :
    ...
    ui->sizeWarningLabel->setText(
            tr("%1 will download and store a copy of the Dogetoreum block chain.").arg(PACKAGE_NAME) + " " +
...
bool Intro::pickDataDirectory(interfaces::Node &node) {
    ...
    if (!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) ||
        ...
        intro.setWindowIcon(QIcon(":icons/dogetoreum"));
     ...
     * override -datadir in the dogetoreum.conf file in the default data directory
     * (to be consistent with dogetoreumd behavior)
     */
    if (dataDir != GUIUtil::getDefaultDataDirectory()) {
