...
public:
    explicit AmountValidator(QObject *parent) :
            QValidator(parent),
            currentUnit(BitcoinUnits::DOGUM),
            assetUnit(-1) {}
...
public:
    ...
    QSize minimumSizeHint() const override {
        ...
        int h = 0;
        int w = GUIUtil::TextWidth(fm, BitcoinUnits::format(BitcoinUnits::DOGUM, BitcoinUnits::maxMoney(), false,
                                                            BitcoinUnits::separatorAlways));
        w += 2; // cursor blinking space
        w += GUIUtil::dogetoreumThemeActive() ? 24 : 0; // counteract padding from css
        return QSize(w, h);
    }
