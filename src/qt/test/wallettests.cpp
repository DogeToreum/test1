...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_dogetoreum-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_dogetoreum-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_dogetoreum-qt  # macOS
    void TestGUI(interfaces::Node &node) {
        ...
        for (QWidget *widget: QApplication::topLevelWidgets()) {
            if (widget->inherits("ReceiveRequestDialog")) {
                ...
                QVERIFY(paymentTextList.at(2).indexOf(QString("URI: dogetoreum:")) != -1);
...
void WalletTests::walletTests() {
    ...
    if (QApplication::platformName() == "minimal") {
      QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke with 'test_dogetoreum-qt -platform cocoa' on mac or use a linux or windows build.");
      return;
    }
#endif
    TestGUI(m_node);
