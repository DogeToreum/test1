    ...
    switch (tab) {
        case SendingTab:
            ui->labelExplanation->setText(
                    tr("These are your Dogetoreum addresses for sending payments. Always check the amount and the receiving address before sending coins."));
            ui->deleteAddress->setVisible(true);
            ui->newAddress->setVisible(true);
            break;
        case ReceivingTab:
            ui->labelExplanation->setText(
                    tr("These are your Dogetoreum addresses for receiving payments. It is recommended to use a new receiving address for each transaction."));
...
void AddressBookPage::on_exportButton_clicked() {
    ...
    if (!writer.write()) {
        //: %1 is a name of the file (e.g.: "addressbook.csv") that the Dogetoreum addresses were exported to.
