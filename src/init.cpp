 ...
 */
#ifndef WIN32
static const char *DOGETOREUM_PID_FILENAME = "dogetoreumd.pid";

static fs::path GetPidFile() {
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", DOGETOREUM_PID_FILENAME)));
}
...
void SetupServerArgs() {
    ...
    gArgs.AddArg("-pid=<file>", strprintf(
            ...
            DOGETOREUM_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#else
    ...
    gArgs.AddArg("-printtoconsole",
                 ...
                 ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg("-pushversion", "Protocol version OVERRIDE to report to other Dogetoreum nodes", ArgsManager::ALLOW_ANY,
                 OptionsCategory::DEBUG_TEST);
    gArgs.AddArg("-shrinkdebugfile", "Shrink debug.log file on client startup (default: 1 when no -debug)",
                 ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg("-sporkaddr=<dogetoreumaddress>",
    ...
    gArgs.AddArg("-platform-user=<user>",
                 "Set the username for the \"platform user\", a restricted user intended to be used by Dogetoreum Platform, to the specified username.",
                 ArgsManager::ALLOW_ANY, OptionsCategory::SMARTNODE);
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE = "<https://github.com/DogeToreum/dogetoreum>";
    const std::string URL_WEBSITE = "<https://dogetoreum.com>";
...
/** Sanity checks
 *  Ensure that Dogetoreum Core is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly) {
    // Make sure only a single Dogetoreum Core process is using the data directory.
...
bool AppInitMain(const util::Ref &context, NodeContext &node, interfaces::BlockAndHeaderTipInfo *tip_info) {
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf(
                ...
                "current working directory '%s'. This is fragile, because if Dogetoreum Core is started in the future "
                "from a different location, it will be unable to locate the current data files. There could "
                "also be data loss if Dogetoreum Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule Dogetoreum-specific tasks
