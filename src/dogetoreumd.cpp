...
static bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/dogetoreum.conf are parsed in qt/dogetoreum.cpp's main()
    SetupServerArgs();
    std::string error;
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage: dogetoreumd [options]           Start "
            PACKAGE_NAME
            " Daemon\n";
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(strprintf(
                        "Error: Command line contains unexpected token '%s', see dogetoreumd -h for a list of options.\n",
                        argv[i]));
            }
        }

        // -server defaults to true for dogetoreumd but not for the GUI so do this here
...
int main(int argc, char *argv[]) {
    ...
    // Connect dogetoreumd signal handlers
    noui_connect();
