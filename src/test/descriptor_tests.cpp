...
/** Compare two descriptors. If only one of them has a checksum, the checksum is ignored. */
        ...
        if (InsecureRandBool()) {
            while (true) {
                ...
                if (it != std::string::npos) {
                    ret[it] = 'h';
                    if (ret.size() > 9 && ret[ret.size() - 9] == '#') ret = ret.substr(0, ret.size() -
                                                                                          9); // Changing apostrophe to h breaks the checksum
                } else {
                    break;
                }
            }
    ...
    void Check(const std::string &prv, const std::string &pub, int flags,
        ...
        std::unique_ptr <Descriptor> parse_priv = Parse(MaybeUseHInsteadOfApostrophy(prv), keys_priv, error);
        std::unique_ptr <Descriptor> parse_pub = Parse(MaybeUseHInsteadOfApostrophy(pub), keys_pub, error);
            ...
            for (int t = 0; t < 2; ++t) {
                ...
                for (size_t n = 0; n < spks.size(); ++n) {
                    ...
                    if (flags & SIGNABLE) {
                        ...
                        spend.vout.resize(1);
                        BOOST_CHECK_MESSAGE(
                                SignSignature(Merge(keys_priv, script_provider), spks[n], spend, 0, 1, SIGHASH_ALL),
                                prv);
                    }
