...
BOOST_AUTO_TEST_CASE(coins_cache_simulation_test)
        ...
        for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
            // Do a random modification.
            {
                ...
                const Coin &entry = (InsecureRandRange(500) == 0) ? AccessByTxid(*stack.back(), txid)
                ...
                if (InsecureRandRange(5) == 0 || coin.IsSpent()) {
                    ...
                    if (InsecureRandRange(16) == 0 && coin.IsSpent()) {
                        ...
                        newcoin.out.scriptPubKey.assign(InsecureRandBits(6),
...
BOOST_AUTO_TEST_CASE(updatecoins_simulation_test)
        ...
        for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
            ...
            if (randiter % 20 < 19) {
                ...
                tx.vout[0].scriptPubKey.assign(InsecureRand32() & 0x3F,
                                               0); // Random sizes so we can test memory usage accounting
...
void CheckAccessCoin(CAmount base_value, CAmount cache_value, CAmount expected_value, char cache_flags,
                     char expected_flags) {
...
void CheckAddCoinBase(CAmount base_value, CAmount cache_value, CAmount modify_value, CAmount expected_value,
...
void
CheckWriteCoins(CAmount parent_value, CAmount child_value, CAmount expected_value, char parent_flags, char child_flags,
                char expected_flags) {
