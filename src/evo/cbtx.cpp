...
bool CheckCbTxMerkleRoots(const CBlock &block, const CBlockIndex *pindex, CValidationState &state,
    ...
    if (pindex) {
        ...
        if (calculatedMerkleRoot != cbTx.merkleRootMNList) {
            std::cout << "calculatedMerkleRoot !=  cbTx.merkleRootMNList " << calculatedMerkleRoot.GetHex() << "!="
                      << cbTx.merkleRootMNList.GetHex() << std::endl;
    ...
    if (quorums == quorumsCached) {
        ...
        for (const auto &p: quorums) {
            ...
            for (const auto &p2: p.second) {
                ...
                llmq::CFinalCommitmentPtr qc = llmq::quorumBlockProcessor->GetMinedCommitment(p.first,
                                                                                              p2->GetBlockHash(),
                                                                                              minedBlockHash);
