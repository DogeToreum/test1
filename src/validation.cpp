...
static bool AcceptToMemoryPoolWorker(const CChainParams &chainparams, CTxMemPool &pool, CValidationState &state,
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin: tx.vin) {
            ...
            if (ptxConflicting) {
                // Transaction conflicts with mempool and RBF doesn't exist in Dogetoreum
...
GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params &consensusParams, bool fSuperblockPartOnly) {
    double nSubsidy = 500000;      // (declaring the reward variable and its original/default amount)
    if (nPrevHeight < 720) {
        nSubsidy = Params().NetworkIDString() == CBaseChainParams::TESTNET ? 20000000 : 1000000;
        ...
        nSubsidy = 8000;
    } else if ((nPrevHeight >= 2500000) && (nPrevHeight < 3000000)) {
        nSubsidy = 4000;
    } else if ((nPrevHeight >= 3000000) && (nPrevHeight < 3500000)) {
        nSubsidy = 2000;
    } else if ((nPrevHeight >= 3500000) && (nPrevHeight < 4500000)) {
        nSubsidy = 1000;
...
static int64_t nTimeDogetoreumSpecific = 0;
...
bool CChainState::ConnectBlock(const CBlock &block, CValidationState &state, CBlockIndex *pindex,
    ...
    /// DOGETOREUM: Check superblock start
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, assetsCache, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(DOGETOREUM): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // DOGETOREUM : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto &tx: block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(DOGETOREUM): transaction %s conflicts with transaction lock %s",
    ...
    // DOGETOREUM : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, (blockReward + specialTxFees), strError)) {
        return state.DoS(0, error("ConnectBlock(DOGETOREUM): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(DOGETOREUM): couldn't find smartnode or superblock payments"),
    ...
    nTimeDogetoreumSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Dogetoreum specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4),
             nTimeDogetoreumSpecific * MICRO, nTimeDogetoreumSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex *pindex, CCoinsViewCache &inputs, const CChainParams &params,
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, assetsCache, false /*fJustCheck*/,
                                  false /*fScriptChecks*/)) {
        return error("RollforwardBlock(DOGUM): ProcessSpecialTxsInBlock for block %s failed with %s",
                     pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
