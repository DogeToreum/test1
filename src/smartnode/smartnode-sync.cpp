...
void CSmartnodeSync::ProcessTick(CConnman &connman) {
    ...
    for (auto &pnode: vNodesCopy) {
        ...
        {
            if ((pnode->fWhitelisted || pnode->m_manual_connection) &&
                !netfulfilledman.HasFulfilledRequest(pnode->addr, strAllow)) {
            ...
            if (!netfulfilledman.HasFulfilledRequest(pnode->addr, "spork-sync")) {
                ...
                LogPrintf(
                        "CSmartnodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- requesting sporks from peer=%d\n",
                        nTick, nCurrentAsset, pnode->GetId());
            }

            if (nCurrentAsset == SMARTNODE_SYNC_BLOCKCHAIN) {
                int64_t nTimeSyncTimeout =
                        vNodesCopy.size() > 3 ? SMARTNODE_SYNC_TICK_SECONDS : SMARTNODE_SYNC_TIMEOUT_SECONDS;
                if (fReachedBestHeader && (GetTime() - nTimeLastBumped > nTimeSyncTimeout)) {
                    ...
                    if (gArgs.GetBoolArg("-syncmempool", DEFAULT_SYNC_MEMPOOL)) {
                        ...
                        for (auto pNodeTmp: vNodesCopy) {
                            bool fRequestedEarlier = netfulfilledman.HasFulfilledRequest(pNodeTmp->addr,
                                                                                         "mempool-sync");
                            if (pNodeTmp->nVersion >= 70216 && !pNodeTmp->fInbound && !fRequestedEarlier) {
                                ...
                                LogPrintf(
                                        "CSmartnodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- syncing mempool from peer=%d\n",
            ...
            if (nCurrentAsset == SMARTNODE_SYNC_GOVERNANCE) {
                ...
                LogPrint(BCLog::GOBJECT,
                         "CSmartnodeSync::ProcessTick -- nTick %d nCurrentAsset %d nTimeLastBumped %lld GetTime() %lld diff %lld\n",
    ...
    for (auto &pnode: vNodesCopy) {
        ...
        if (nObjsLeftToAsk == 0) {
            ...
            if (GetTime() - nTimeNoObjectsLeft > SMARTNODE_SYNC_TIMEOUT_SECONDS &&
                ...
                std::max(int(0.0001 * nLastVotes), SMARTNODE_SYNC_TICK_SECONDS)) {
                LogPrintf(
                        "CSmartnodeSync::ProcessTick -- nTick %d nCurrentAsset %d -- asked for all objects, nothing to do\n",
...
void CSmartnodeSync::UpdatedBlockTip(const CBlockIndex *pindexNew, bool fInitialDownload, CConnman &connman) {
    ...
    LogPrint(BCLog::MNSYNC,
             "CSmartnodeSync::UpdatedBlockTip -- pindexNew->nHeight: %d pindexTip->nHeight: %d fInitialDownload=%d fReachedBestHeader=%d\n",
