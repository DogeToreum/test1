...
public:
    ...
    constexpr CCoinJoinStatusUpdate(int nSessionID, PoolState nState, int nEntriesCount, PoolStatusUpdate nStatusUpdate,
...
public:
    ...
    CCoinJoinEntry() : txCollateral(MakeTransactionRef()) {}
...
public:
    ...
    std::string ToString() const {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, smartnode=%s", nDenom, nTime,
                         fReady ? "true" : "false", fTried ? "true" : "false", smartnodeOutpoint.ToStringShort());
    }

    friend bool operator==(const CCoinJoinQueue &a, const CCoinJoinQueue &b) {
        return a.nDenom == b.nDenom && a.smartnodeOutpoint == b.smartnodeOutpoint && a.nTime == b.nTime &&
               a.fReady == b.fReady;
    }
};
...
public:
    ...
    CCoinJoinBroadcastTx() : tx(MakeTransactionRef()) {}
...
protected:
    ...
    std::vector <CCoinJoinEntry> vecEntries
    GUARDED_BY(cs_coinjoin); // Smartnode/clients entries
    ...
    GUARDED_BY(cs_coinjoin); // the finalized transaction ready for signing

    void SetNull()

    EXCLUSIVE_LOCKS_REQUIRED(cs_coinjoin);

    bool IsValidInOuts(const std::vector <CTxIn> &vin, const std::vector <CTxOut> &vout, PoolMessage &nMessageIDRet,
...
public:
    ...
    LOCKS_EXCLUDED(cs_coinjoin) {LOCK(cs_coinjoin); return vecEntries.size();}

    int GetEntriesCountLocked() const

    EXCLUSIVE_LOCKS_REQUIRED(cs_coinjoin) {return vecEntries.size();}
};
...
public:
    ...
    LOCKS_EXCLUDED(cs_vecqueue) {LOCK(cs_vecqueue); return vecCoinJoinQueue.size();}
...
private:
    ...
    // static members
    static constexpr std::array<CAmount, 5>
    ...
    GUARDED_BY(cs_mapdstx);

    static void CheckDSTXes(const CBlockIndex *pindex)

    LOCKS_EXCLUDED(cs_mapdstx);

public:
    static constexpr std::array<CAmount, 5>

    GetStandardDenominations() { return vecStandardDenominations; }

    static constexpr CAmount

    GetSmallestDenomination() { return vecStandardDenominations.back(); }
    ...
    /**
     * Return a bitshifted integer representing a denomination
     * in vecStandardDenominations or 0 if none was found
     */
    ...
    DenominationToAmount(int nDenom) {
        if (nDenom == 0) {
    ...
    GetMaxPoolAmount() { return COINJOIN_ENTRY_MAX_SIZE * vecStandardDenominations.front(); }
    ...
    GetCollateralAmount() { return GetSmallestDenomination() / 10; }

    static constexpr CAmount

    GetMaxCollateralAmount() { return GetCollateralAmount() * 4; }

    static constexpr bool IsCollateralAmount(CAmount nInputAmount) {
        // collateral input can by anything between 1x and "max" (including both)
    ...
    static constexpr int CalculateAmountPriority(CAmount nInputAmount) {
        ...
        if (nInputAmount < COIN) { return 20000; }

        // non-denoms return largest first
    ...
    static void BlockConnected(const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindex,
