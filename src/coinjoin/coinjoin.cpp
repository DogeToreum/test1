...
bool CCoinJoinQueue::Sign() {
    ...
    CBLSSignature
    sig = WITH_LOCK(activeSmartnodeInfoCs,
    return activeSmartnodeInfo.blsKeyOperator->Sign(hash));
    if (!sig.IsValid()) {
        return false;
    }
...
bool CCoinJoinBaseSession::IsValidInOuts(const std::vector <CTxIn> &vin, const std::vector <CTxOut> &vout,
    ...
    auto checkTxOut = [&](const CTxOut &txout) {
        ...
        if (nDenom != nSessionDenom) {
            LogPrint(BCLog::COINJOIN,
                     "CCoinJoinBaseSession::IsValidInOuts -- ERROR: incompatible denom %d (%s) != nSessionDenom %d (%s)\n",
                     nDenom, CCoinJoin::DenominationToString(nDenom), nSessionDenom,
        ...
        if (!setScripPubKeys.insert(txout.scriptPubKey).second) {
            LogPrint(BCLog::COINJOIN,
                     "CCoinJoinBaseSession::IsValidInOuts -- ERROR: already have this script! scriptPubKey=%s\n",
                     ScriptToAsmStr(txout.scriptPubKey));
    ...
    CCoinsViewMemPool
    viewMemPool(WITH_LOCK(cs_main,
    return &::ChainstateActive().CoinsTip()), mempool);
    ...
    for (const auto &txin: vin) {
        ...
        if (!viewMemPool.GetCoin(txin.prevout, coin) || coin.IsSpent() ||
            ...
            LogPrint(BCLog::COINJOIN,
                     "CCoinJoinBaseSession::%s -- ERROR: missing, spent or non-locked mempool input! txin=%s\n",
                     __func__, txin.ToString());
...
GUARDED_BY(CCoinJoin::cs_mapdstx);
...
bool CCoinJoin::IsCollateralValid(const CTransaction &txCollateral) {
    ...
    for (const auto &txout: txCollateral.vout) {
        ...
        if (!txout.scriptPubKey.IsPayToPublicKeyHash() && !txout.scriptPubKey.IsUnspendable()) {
            LogPrint(BCLog::COINJOIN, "CCoinJoin::IsCollateralValid -- Invalid Script, txCollateral=%s",
    ...
    for (const auto &txin: txCollateral.vin) {
        ...
        if (mempoolTx != nullptr) {
            if (mempool.isSpent(txin.prevout) || !llmq::quorumInstantSendManager->IsLocked(txin.prevout.hash)) {
                LogPrint(BCLog::COINJOIN,
                         "CCoinJoin::IsCollateralValid -- spent or non-locked mempool input! txin=%s\n",
                         txin.ToString());
                return false;
            }
            ...
            LogPrint(BCLog::COINJOIN,
                     "CCoinJoin::IsCollateralValid -- Unknown inputs in collateral transaction, txCollateral=%s",
                     txCollateral.ToString()); /* Continued */
            return false;
        }
    }
    ...
    if (nValueIn - nValueOut < GetCollateralAmount()) {
        LogPrint(BCLog::COINJOIN,
                 "CCoinJoin::IsCollateralValid -- did not include enough fees in transaction: fees: %d, txCollateral=%s",
                 nValueOut - nValueIn, txCollateral.ToString()); /* Continued */
        return false;
    }
...

    {
        ...
        if (!AcceptToMemoryPool(mempool, validationState, MakeTransactionRef(txCollateral), /*pfMissingInputs=*/
                                nullptr, /*bypass_limits=*/false, /*nAbsurdFee=*/DEFAULT_MAX_RAW_TX_FEE, /*fDryRun=*/
                                true)) {
