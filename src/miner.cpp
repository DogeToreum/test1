...
void static DogetoreumMiner(const CChainParams& chainparams, NodeContext& node) {
    LogPrintf("DogetoreumMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("dogetoreum-miner");
    ...
    unsigned int nExtraNonce = 0;
        ...
        //     LogPrintf("DogetoreumMiner -- Wallet not available\n");
        // }
    #endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf(
                        "DogetoreumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("DogetoreumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("DogetoreumMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("DogetoreumMiner -- runtime error: %s\n", e.what());
        return;
    }
}

// TODO: add reference node, get the conn man from there
int GenerateDogetoreums(bool fGenerate, int nThreads, const CChainParams &chainparams, NodeContext &node) {
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(
                boost::bind(&DogetoreumMiner, boost::cref(chainparams), boost::ref(node)));
    }
    return (numCores);
}