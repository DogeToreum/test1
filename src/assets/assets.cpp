...
static const std::regex dogum_names("^DOGUM$|^DOGETOREUM$|^wDOGUM$|^WDOGUM$|^DOGUMcoin$|^DOGUMCOIN$");


bool IsAssetNameValid(std::string name, bool isRoot) {
    if (name.length() < 3 || name.length() > 128) return false;
    if (isRoot)
        return std::regex_match(name, name_root_characters) && !std::regex_match(name, dogum_names);
    else
        return std::regex_match(name, name_sub_characters) && !std::regex_match(name, dogum_names); 
}
