...
bool CScript::IsAssetScript(int &nIndex) const {
    if (this->size() > 31) {
        if ((*this)[25] == OP_ASSET_ID) { // OP_DOGUM_ASSET is always in the 25 index of the script if it exists
            nIndex = -1;
            if ((*this)[27] == DOGUM_D) { // Check to see if DOGUM starts at 27 ( this->size() < 105)
                if ((*this)[28] == DOGUM_0)
                    if ((*this)[29] == DOGUM_G)
                        nIndex = 30;
            } else {
                if ((*this)[28] == DOGUM_D) // Check to see if DOGUM starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == DOGUM_0)
                        if ((*this)[30] == DOGUM_G)
                            nIndex = 31;
            }
