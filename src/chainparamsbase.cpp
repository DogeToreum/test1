...
std::unique_ptr <CBaseChainParams> CreateBaseChainParams(const std::string &chain) {
    if (chain == CBaseChainParams::MAIN)
        return MakeUnique<CBaseChainParams>("", 10419);
    else if (chain == CBaseChainParams::TESTNET)
        return MakeUnique<CBaseChainParams>("rip01-testnet", 10521);
    else if (chain == CBaseChainParams::DEVNET)
        return MakeUnique<CBaseChainParams>(gArgs.GetDevNetName(), 19698);
    else if (chain == CBaseChainParams::REGTEST)
        return MakeUnique<CBaseChainParams>("regtest", 19798);
    else
