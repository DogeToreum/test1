...
// To continue using `generate` in v0.17, restart dogetoreumd with the
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"generatetoaddress",
        "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
        {
            ...
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated DOGUM to."},
            {"maxtries", RPCArg::Type::NUM, /* default */ "1000000", "How many iterations to try."},
        },
        RPCResult{
            RPCResult::Type::ARR, "", "hashes of blocks generated",
        ...
        RPCExamples{
            ...
            + "If you are running the Dogetoreum Core wallet, you can get a new address to send the newly generated coins to with:\n"
...
static UniValue getblocktemplate(const JSONRPCRequest &request) {
    ...
    if (::ChainstateActive().IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Dogetoreum Core is downloading blocks...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (AreSuperblocksEnabled()
        && !smartnodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(::ChainActive().Height() + 1))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Dogetoreum Core is syncing with network...");
...
UniValue setgenerate(const JSONRPCRequest &request) {
    ...
    int numCores = GenerateDogetoreums(fGenerate, nGenProcLimit, Params(), node);
