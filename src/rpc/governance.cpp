...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 DOGETOREUM irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on dogetoreumcentral etc. and do not save them...
...
void gobject_vote_conf_help(const JSONRPCRequest &request) {
    RPCHelpMan{"gobject vote-conf",
               "Vote on a governance object by smartnode configured in dogetoreum.conf\n",
               {
...
static UniValue gobject_vote_conf(const JSONRPCRequest &request) {
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("dogetoreum.conf", statusObj);
...
[[noreturn]] void gobject_help() {
    RPCHelpMan{"gobject",
        ...
        "  list-prepared      - Returns a list of governance objects prepared by this wallet with \"gobject prepare\"\n"
               ...
               #endif // ENABLE_WALLET
               "  vote-conf          - Vote on a governance object by smartnode configured in dogetoreum.conf\n"
#ifdef ENABLE_WALLET
...
static const CRPCCommand commands[] =
        { //  category              name                      actor (function)         argNames
                //  --------------------- ------------------------  -----------------------  ----------
                /* Dogetoreum features */
                {"dogetoreum", "getgovernanceinfo",   &getgovernanceinfo,   {}},
                {"dogetoreum", "getsuperblockbudget", &getsuperblockbudget, {"index"}},
                {"dogetoreum", "gobject",             &gobject,             {}},
                {"dogetoreum", "voteraw",             &voteraw,             {"tx_hash", "tx_index", "gov_hash", "signal", "outcome", "time", "sig"}},
