...
GUARDED_BY(cs_rpcWarmup) = true;
static std::string rpcWarmupStatus
GUARDED_BY(cs_rpcWarmup) = "RPC server started";
...
static bool
ExecuteCommand(const CRPCCommand &command, const JSONRPCRequest &request, UniValue &result, bool last_handler,
...
struct RPCServerInfo {
    Mutex mutex;
    std::list <RPCCommandExecutionInfo> active_commands
    GUARDED_BY(mutex);
};
...
struct RPCCommandExecution {
    ...
    explicit RPCCommandExecution(const std::string &method) {
        ...
        it = g_rpc_server_info.active_commands.insert(g_rpc_server_info.active_commands.cend(),
...
std::string
CRPCTable::help(const std::string &strCommand, const std::string &strSubCommand, const JSONRPCRequest &helpreq) const {
    std::string strRet;
    std::string category;
...
static RPCHelpMan help() {
    return RPCHelpMan{"help",
                      "\nList all commands, or get help for a specified command.\n",
                      {
                              ...
                              {"subcommand", RPCArg::Type::STR, /* default */ "all subcommands",
...
transformNamedArguments(const JSONRPCRequest &in, const std::vector <std::string> &argNames) {
        ...
        for (auto itRequest = it.first; itRequest != it.second; ++itRequest) {
            ...
            if (nRestrictedParamsCount > 0) {
                if (request.params.empty()) {
                    throw JSONRPCError(RPC_PLATFORM_RESTRICTION,
                                       strprintf("Method \"%s\" has parameter restrictions.", request.strMethod));
                }
                ...
                for (size_t i = 0; i < nRestrictedParamsCount; ++i) {
                    if (request.params[i].type() != itRequest->second[i].type() ||
                        request.params[i].getValStr() != itRequest->second[i].getValStr()) {
                        LogPrint(BCLog::RPC,
                                 "CRPCTable::%s: Method \"%s\" requires parameter %d to be %s with type %d\n", __func__,
                                 request.strMethod, i, itRequest->second[i].getValStr(), itRequest->second[i].type());
                        fMatch = false;
                    }
                }
