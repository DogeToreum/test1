...
const char *const BITCOIN_CONF_FILENAME = "dogetoreum.conf";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager &am, const std::string &arg) {
        ...
        // argument value seen from the command line (so "dogetoreumd -foo=bar
...
fs::path GetDefaultDataDir() {
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DogetoreumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DogetoreumCore
    // Mac: ~/Library/Application Support/DogetoreumCore
    // Unix: ~/.dogetoreumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DogetoreumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DogetoreumCore";
#else
    // Unix
    return pathRet / ".dogetoreumcore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string &error, bool ignore_invalid_keys) {
    ...
    if (stream.good()) {
        ...
        // Create an empty dogetoreum.conf if it does not exist
...
std::string CopyrightHolders(const std::string &strPrefix, unsigned int nStartYear, unsigned int nEndYear) {
    ...
    // Check for untranslated substitution to make sure Dogetoreum Core copyright is not removed by accident
    if (copyright_info.find("Dogetoreum") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2020, nEndYear) + "The Dogetoreum developers";
    }
