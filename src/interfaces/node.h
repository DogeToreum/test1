...
//! Interface for the src/evo part of a dogetoreum node (dogetoreumd process).
    class EVO {
    public:
        virtual ~EVO() {}
...
//! Interface for the src/llmq part of a dogetoreum node (dogetoreumd process).
    class LLMQ {
    public:
        virtual ~LLMQ() {}
...
//! Interface for the src/smartnode part of a dogetoreum node (dogetoreumd process).
    namespace Smartnode {
        class Sync {
        public:
            virtual ~Sync() {}
...
//! Top-level interface for a dogetoreum node (dogetoreumd process).
    class Node {
    public:
        virtual ~Node() {}
        ...
        using MessageBoxFn = std::function<bool(const std::string &message, const std::string &caption,
                                                unsigned int style)>;

        virtual std::unique_ptr <Handler> handleMessageBox(MessageBoxFn fn) = 0;

        //! Register handler for question messages.
        using QuestionFn = std::function<bool(const std::string &message, const std::string &non_interactive_message,
        ...
        using NotifyBlockTipFn = std::function<void(bool initial_download, int height, int64_t block_time,
                                                    const std::string &block_hash, double verification_progress)>;

        virtual std::unique_ptr <Handler> handleNotifyBlockTip(NotifyBlockTipFn fn) = 0;

        //! Register handler for chainlock messages.
        using NotifyChainLockFn = std::function<void(const std::string &bestChainLockedHash,
                                                     int32_t bestChainLockedHeight)>;

        virtual std::unique_ptr <Handler> handleNotifyChainLock(NotifyChainLockFn fn) = 0;

        //! Register handler for header tip messages.
        using NotifyHeaderTipFn = std::function<void(bool initial_download, int height, int64_t block_time,
                                                     const std::string &block_hash, double verification_progress)>;
        ...
        handleNotifyAdditionalDataSyncProgressChanged(NotifyAdditionalDataSyncProgressChangedFn fn) = 0;
