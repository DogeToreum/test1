    ...
    namespace {
        ...
        public:
            ...
            std::unique_ptr <Handler> handleNotifyBlockTip(NotifyBlockTipFn fn) override {
                return MakeHandler(
                        ::uiInterface.NotifyBlockTip_connect([fn](bool initial_download, const CBlockIndex *block) {
            ...
            std::unique_ptr <Handler> handleNotifyChainLock(NotifyChainLockFn fn) override {
                return MakeHandler(::uiInterface.NotifyChainLock_connect(
                        [fn](const std::string &bestChainLockHash, int bestChainLockHeight) {
            ...
            std::unique_ptr <Handler> handleNotifySmartnodeListChanged(NotifySmartnodeListChangedFn fn) override {
                return MakeHandler(
                        ::uiInterface.NotifySmartnodeListChanged_connect([fn](const CDeterministicMNList &newList) {
                            fn(newList);
                        }));
            }

            std::unique_ptr <Handler>
            handleNotifyAdditionalDataSyncProgressChanged(NotifyAdditionalDataSyncProgressChangedFn fn) override {
                return MakeHandler(
