...
bool BerkeleyEnvironment::Open(bool retry) {
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of Dogetoreum may be using it.\n",
                  strPath);
        return false;
    }
...
bool BerkeleyDatabase::Verify(std::string &errorStr) {
    ...
    if (fs::exists(file_path)) {
        ...
        if (result != 0) {
            errorStr = strprintf(
                    _("%s corrupt. Try using the wallet tool dogetoreum-wallet to salvage or restoring a backup."),
                    file_path);
            return false;
        }
    }
