...
#ifndef DOGETOREUM_CRYPTO_BLS_WORKER_H
#define DOGETOREUM_CRYPTO_BLS_WORKER_H
...
private:
    ...
    struct SigVerifyJob {
        ...
        SigVerifyJob(SigVerifyDoneCallback &&_doneCallback, CancelCond &&_cancelCond, const CBLSSignature &_sig,
                     const CBLSPublicKey &_pubKey, const uint256 &_msgHash) :
...
public:
    ...
    bool GenerateContributions(int threshold, const BLSIdVector &ids, BLSVerificationVectorPtr &vvecRet,
    ...
    AsyncBuildQuorumVerificationVector(const std::vector <BLSVerificationVectorPtr> &vvecs,
                                       size_t start, size_t count, bool parallel);
    ...
    CBLSSecretKey
    AggregateSecretKeys(const BLSSecretKeyVector &secKeys, size_t start = 0, size_t count = 0, bool parallel = true);
    ...
    void AsyncVerifyContributionShares(const CBLSId &forId, const std::vector <BLSVerificationVectorPtr> &vvecs,
    ...
    AsyncVerifyContributionShares(const CBLSId &forId, const std::vector <BLSVerificationVectorPtr> &vvecs,
                                  const BLSSecretKeyVector &skShares,
                                  bool parallel, bool aggregated);

    std::vector<bool> VerifyContributionShares(const CBLSId &forId, const std::vector <BLSVerificationVectorPtr> &vvecs,
                                               const BLSSecretKeyVector &skShares,
                                               bool parallel = true, bool aggregated = true);

    std::future<bool> AsyncVerifyContributionShare(const CBLSId &forId, const BLSVerificationVectorPtr &vvec,
    ...
    static bool
    VerifyVerificationVectors(const std::vector <BLSVerificationVectorPtr> &vvecs, size_t start = 0, size_t count = 0);
    ...
    void AsyncVerifySig(const CBLSSignature &sig, const CBLSPublicKey &pubKey, const uint256 &msgHash,
                        SigVerifyDoneCallback doneCallback, CancelCond cancelCond = [] { return false; });

    std::future<bool> AsyncVerifySig(const CBLSSignature &sig, const CBLSPublicKey &pubKey, const uint256 &msgHash,
...
public:
    ...
    BuildQuorumVerificationVector(const uint256 &cacheKey, const std::vector <BLSVerificationVectorPtr> &vvecs) {
...
#endif //DOGETOREUM_CRYPTO_BLS_WORKER_H
