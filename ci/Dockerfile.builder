...
# dogetoreum_hash
RUN git clone https://github.com/dogetoreum/dogetoreum_hash
RUN cd dogetoreum_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} dogetoreum
RUN useradd -u ${USER_ID} -g dogetoreum -s /bin/bash -m -d /dogetoreum dogetoreum
...
RUN mkdir /dogetoreum-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /dogetoreum-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /dogetoreum-src
