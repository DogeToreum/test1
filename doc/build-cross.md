Cross-compiliation of Dogetoreum Core
===============================

Dogetoreum Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Dogetoreum.
...
When building Dogetoreum Core, use
...
Note that for WSL the Dogetoreum Core source path MUST be somewhere in the default mount file system, for
example /usr/src/dogetoreum, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Dogetoreum Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Dogetoreum Core source code).
